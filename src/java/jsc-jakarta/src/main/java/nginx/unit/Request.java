...
public class Request implements HttpServletRequest, DynamicPathRequest
{
    ...
    public String getContentType()
    {
        ...
        */
        if (characterEncoding == null && content_type != null) {
	        int charsetPos = content_type.indexOf(";charset=");
	        if (charsetPos >= 0) {
	        	characterEncoding = content_type.substring(charsetPos+9);
	        }
        }
    ...
    */
    ...
    public Map<String,String[]> getParameterMap()
    {
        ...
        for(String k : getParameters().keySet()) {
        	List<String> vals = getParameters().get(k);
        	map.put(k,  getParameters().get(k).toArray(new String[vals.size()]));
        }
